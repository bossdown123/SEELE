...
class Trader:
    def __init__(self, model, multi_scaler, encoder, trading_client=None, history_client=None):
        ...
        self.trading_client = TradingClient(self.apikey, self.secret, paper=True) if trading_client is None else trading_client
        self.client = StockHistoricalDataClient(self.apikey, self.secret) if history_client is None else history_client
        self.options_client = OptionHistoricalDataClient(self.apikey, self.secret)
    ...
    def get_bars_multi(self, symbol_or_symbols, time):
        with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
            tasks = [pool.apply_async(self.get_bars, (symbol, time)) for symbol in symbol_or_symbols]
